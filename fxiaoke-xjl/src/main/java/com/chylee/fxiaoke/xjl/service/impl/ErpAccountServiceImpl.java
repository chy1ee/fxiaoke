...
public class ErpAccountServiceImpl implements ErpAccountService {
    ...
    public ErpAccountServiceImpl(CopmaMapper copmaMapper) {
    ...
    private AccountRespEvent addKehu(AccountReqEvent reqEvent) {
        ...
        try {
            ...
            return new AccountRespEvent(khbh);
        }
        catch(Exception e) {
    ...
    public Copma toCopma(AccountObj obj, ContactObj contactObj, List<AccountAddrObj> accountAddrObjs) {
        ...
        copma.setMA002(obj.getField_e4t2b__c());	//客户简称
        copma.setMA003(obj.getName());	//客户全称
        copma.setMA004(null);	//负责人
        if (contactObj != null) {
            copma.setMA005(contactObj.getName());    //联系人
            copma.setMA006(contactObj.getMobile1());    //TEL_NO(一)
            copma.setMA007(contactObj.getTel1());    //TEL_NO(二)
            copma.setMA008(contactObj.getTel2());    //FaX_NO
        }
        copma.setMA009(null);	//E-Mail
        copma.setMA010(obj.getField_20jT0__c());	//税号
        ...
        copma.setMA016(obj.getYwry());	//业务人员
        ...
        if (accountAddrObjs != null) {
            for (AccountAddrObj accountAddrObj : accountAddrObjs) {
                if (accountAddrObj.isIs_default_add())
                    copma.setMA023(accountAddrObj.getAddress());	//注册地址(一)

                if (accountAddrObj.isIs_ship_to_add())
                    copma.setMA027(accountAddrObj.getAddress());	//送货地址(一)
            }
        }
        ...
        copma.setMA085(obj.getYwry());	//收款业务员
